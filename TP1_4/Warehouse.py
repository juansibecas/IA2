...
class Warehouse:
    ...
    """DEPRECATED (al final tenia un costo computacional mucho mas alto)
    def check_file(self, pos1, pos2):
        try:
            with open(f'Warehouse {self.columns} {self.rows} {self.dx} {self.dy}.txt', 'r', encoding="utf8") as f:
                self.file = f.readlines()
                for line in self.file:
                    start_string_1 = f'[{pos1[0]},{pos1[1]}] to [{pos2[0]},{pos2[1]}]' #estos 2 son lo mismo cambiando el sentido de viaje
                    start_string_2 = f'[{pos2[0]},{pos2[1]}] to [{pos1[0]},{pos1[1]}]'
                    if line.startswith(start_string_1) or line.startswith(start_string_2): #se fija si hay algun string que arranque con eso
                        line_split = line.split(':') 
                        return int(line_split[1])  
        except:
            return 0
        
    def write_to_file(self, pos1, pos2, cost):
        with open(f'Warehouse {self.columns} {self.rows} {self.dx} {self.dy}.txt', 'a', encoding="utf8") as f:
            f.write(f'[{pos1[0]},{pos1[1]}] to [{pos2[0]},{pos2[1]}]:{cost}') #[1,3] to [2,4]:58 es el formato de linea
            f.write("\n")"""